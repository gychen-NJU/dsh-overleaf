...
| Client module id | `dsh-overleaf-workbench` | `dsh-overleaf` (better-overleaf banner) |
...
Build produces `lib/index.js` (node half, ESM) and `lib/client.js` (browser half, lazy-CJS closure registered via `window.__ModuleLoader__.load({ id:'dsh-overleaf-workbench', factory })`). Only React (+jsx-runtime) may be required from platform seeds in the client bundle; everything else inlines — the purity gate mirrors the community convention.
