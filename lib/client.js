window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const WORKBENCH_HOME = "/overleaf-proxy/";
		const WORKBENCH_SETTINGS_CHANGED = "dsh-overleaf:settings-changed";
		function workbenchEntry(embedUrl) {
			if (embedUrl?.startsWith("/overleaf-proxy/") && !/[\\\r\n]/.test(embedUrl)) {
				const parsed = new URL(embedUrl, "http://workbench.invalid");
				if (parsed.pathname.startsWith("/overleaf-proxy/")) return parsed.pathname + parsed.search + parsed.hash;
			}
			return WORKBENCH_HOME;
		}
		function navigateToWorkbenchHome(frame, embedUrl) {
			if (frame !== null) frame.src = workbenchEntry(embedUrl);
		}
		/** Preserve an open project on tab switches, but never on upstream switches. */
		function updateFrameUpstream(frame, origin, embedUrl) {
			const previous = frame.dataset.upstreamOrigin;
			frame.dataset.upstreamOrigin = origin;
			if (previous === void 0 || previous === origin) return false;
		...
		*/
		function parseCompileLog(text) {
			...
			for (const rawLine of lines) {
				...
				if (/(?:^|\b)(?:LaTeX3?|LaTeX\s+Font|Package(?:\s+\S+)?|Class(?:\s+\S+)?|Module(?:\s+\S+)?)\s+Warning|Overfull|Underfull|(?:pdf|Lua|Xe)?TeX warning|Warning:\s/i.test(line) || /(?:Citation|Reference)\s+.*undefined/i.test(line) || /There were undefined references|Label\(s\) may have changed/i.test(line)) push({
					level: "warning",
					message: line.slice(0, 300)
				});
			}
			...
			for (const item of items) {
				...
				if (existing === void 0) {
					...
					unique.push(item);
				} else if (existing.file === void 0 && item.file !== void 0) existing.file = item.file;
			}
			return {
				items: unique,
				errors: unique.filter((item) => item.level === "error").length,
				warnings: unique.filter((item) => item.level === "warning").length
			};
		}
...
`;
		...
		function OverleafView(props) {
			...
			const tt = (0, react.useMemo)(() => tr ?? ((key) => String(key)), [tr]);
			...
			const [selectionSafetyEnabled, setSelectionSafetyEnabled] = (0, react.useState)(true);
			const [bibPath, setBibPath] = (0, react.useState)("");
			const [bibCandidates, setBibCandidates] = (0, react.useState)([]);
			const [bibScanState, setBibScanState] = (0, react.useState)("idle");
			const [bibBusy, setBibBusy] = (0, react.useState)(false);
			const [texPath, setTexPath] = (0, react.useState)("");
			const [texCandidates, setTexCandidates] = (0, react.useState)([]);
			const [texScanState, setTexScanState] = (0, react.useState)("idle");
			const [texDirection, setTexDirection] = (0, react.useState)("overleaf-to-local");
			const [texManualContent, setTexManualContent] = (0, react.useState)("");
			const [texReverseConfirmed, setTexReverseConfirmed] = (0, react.useState)(false);
			const [texBusy, setTexBusy] = (0, react.useState)(false);
			...
			const bibSyncTimerRef = (0, react.useRef)(void 0);
			const texSyncTimerRef = (0, react.useRef)(void 0);
			const texSyncPendingRef = (0, react.useRef)(void 0);
			...
			(0, react.useEffect)(() => {
				let disposed = false;
				const refresh = () => {
					postWorkbench("/overleaf/workbench/embed-info").then((info) => {
						if (!disposed) setEmbedInfo(info);
					}).catch((error) => {
						if (!disposed) setNote({
							ok: false,
					...
					postWorkbench("/overleaf/workbench/status").then((value) => {
				...
				return () => {
					disposed = true;
					window.removeEventListener(WORKBENCH_SETTINGS_CHANGED, refresh);
				};
			}, [tt]);
			(0, react.useEffect)(() => {
				const frame = frameRef.current;
				if (frame === null || embedInfo === void 0) return;
					...
					switch (data.type) {
						...
						case "selection-replace-done": {
							...
							if (done.ok === true) {
								...
								setNote({
									ok: true,
									text: done.forced === true ? tt("selection.replacedForced") : tt("selection.replaced")
								});
							} else setNote({
								ok: false,
						...
						case "bib-sync-done": {
							const done = data;
							if (bibSyncTimerRef.current !== void 0) {
								window.clearTimeout(bibSyncTimerRef.current);
								bibSyncTimerRef.current = void 0;
							}
							setBibBusy(false);
							if (done.ok === true) {
								...
								setNote({
									ok: true,
									text: tt(key).replace("{name}", done.target ?? ".bib").replace("{chars}", String(done.chars ?? 0))
								});
							} else {
								const reasonKey = {
								...
								}[done.error ?? ""] ?? (done.error === "bib-editor-timeout" ? "bib.errorEditorTimeout" : done.error === "bib-editor-unavailable" ? "bib.errorEditorUnavailable" : done.error === "bib-editor-ambiguous" ? "bib.errorEditorAmbiguous" : done.error === "bib-save-timeout" ? "bib.errorSaveTimeout" : done.error === "bib-snapshot-failed" ? "bib.errorSnapshot" : done.error === "bib-target-missing" ? "bib.errorTargetMissing" : done.error === "bib-target-ambiguous" ? "bib.errorTargetAmbiguous" : void 0);
								const diagnostic = done.diagnostic;
								const diagnosticPhase = diagnostic?.phase === "tree" ? "bib.phaseTree" : diagnostic?.phase === "baseline" ? "bib.phaseBaseline" : diagnostic?.phase === "save" ? "bib.phaseSave" : void 0;
								const diagnosticText = diagnosticPhase !== void 0 ? [
									tt(diagnosticPhase),
									diagnostic?.status !== void 0 && Number.isInteger(diagnostic.status) && diagnostic.status >= 100 && diagnostic.status <= 599 ? `HTTP ${diagnostic.status}` : void 0,
									typeof diagnostic?.code === "string" && /^[a-z0-9-]{1,48}$/.test(diagnostic.code) ? diagnostic.code : void 0,
									diagnostic?.attempts === 2 ? tt("bib.readRetried") : void 0
								].filter(Boolean).join(" / ") : void 0;
								const detail = [
									reasonKey !== void 0 ? tt(reasonKey) : done.error ?? "unknown",
									diagnosticText,
									done.available !== void 0 && done.available !== "" ? tt("bib.available").replace("{files}", done.available) : void 0,
									done.written === true ? tt("bib.writeUnconfirmed") : void 0
								].filter(Boolean).join(" · ");
								setNote({
									ok: false,
									text: tt("bib.failed").replace("{detail}", detail)
								});
							}
							return;
						}
						case "tex-document": {
							...
							if (pending === void 0 || doc.requestId !== pending.requestId) return;
							if (texSyncTimerRef.current !== void 0) {
								window.clearTimeout(texSyncTimerRef.current);
								texSyncTimerRef.current = void 0;
							}
							if (doc.ok !== true || typeof doc.id !== "string" || typeof doc.name !== "string" || typeof doc.text !== "string" || typeof doc.revision !== "string") {
								texSyncPendingRef.current = void 0;
								setTexBusy(false);
								const detail = doc.error === "tex-current-document-not-tex" ? tt("tex.errorCurrentNotTex") : doc.error === "tex-document-identity-unavailable" || doc.error === "tex-document-identity-mismatch" ? tt("tex.errorIdentity") : doc.error === "tex-editor-unavailable" || doc.error === "tex-editor-ambiguous" ? tt("tex.errorEditorUnavailable") : doc.error === "tex-document-too-large" ? tt("tex.errorTooLarge") : doc.error ?? "unknown";
								setNote({
									ok: false,
									text: tt("tex.failed").replace("{detail}", detail)
								});
								return;
							}
							if (pending.direction === "overleaf-to-local") {
								postWorkbench("/overleaf/workbench/write-tex-file", {
									...
									setNote({
										ok: false,
										text: tt("tex.failed").replace("{detail}", String(error))
									});
								}).finally(() => {
									if (texSyncPendingRef.current?.requestId !== pending.requestId) return;
									texSyncPendingRef.current = void 0;
							...
							const forwardToOverleaf = (content) => {
								if (texSyncPendingRef.current?.requestId !== pending.requestId) return;
								sendToFrame({
									type: "sync-tex-to-overleaf",
								...
								texSyncTimerRef.current = window.setTimeout(() => {
									...
									setNote({
										ok: false,
										text: tt("tex.failed").replace("{detail}", tt("tex.errorTimeout"))
									});
								}, 45e3);
							};
							if (pending.manualText !== "") forwardToOverleaf(pending.manualText);
							else postWorkbench("/overleaf/workbench/read-tex-file", {
								sessionId: pending.sessionId,
								path: pending.path,
								fallbackName: doc.name
							}, 3e4).then((file) => {
						...
						case "tex-overleaf-sync-done": {
							const done = data;
							const pending = texSyncPendingRef.current;
							if (pending === void 0 || done.requestId !== pending.requestId) return;
							if (texSyncTimerRef.current !== void 0) {
								window.clearTimeout(texSyncTimerRef.current);
								texSyncTimerRef.current = void 0;
							}
							...
							if (done.ok === true) {
								...
								setNote({
									ok: true,
									text: tt(done.unchanged === true ? "tex.overleafUnchanged" : "tex.overleafUpdated").replace("{name}", done.target ?? ".tex").replace("{chars}", String(done.chars ?? 0))
								});
							} else {
								const detail = done.error === "tex-remote-changed" ? tt("tex.errorRemoteChanged") : done.error === "tex-sync-busy" ? tt("tex.errorBusy") : done.error === "tex-save-timeout" ? tt("tex.errorSaveTimeout") : done.error === "tex-snapshot-failed" ? tt("tex.errorSnapshot") : done.error === "tex-current-document-not-tex" ? tt("tex.errorCurrentNotTex") : done.error === "tex-document-identity-unavailable" || done.error === "tex-document-identity-mismatch" ? tt("tex.errorIdentity") : done.error === "tex-editor-unavailable" || done.error === "tex-editor-ambiguous" ? tt("tex.errorEditorUnavailable") : done.error === "tex-write-verification-failed" ? tt("tex.errorVerification") : done.error === "tex-remote-read-failed" ? tt("tex.errorReadback") : done.error ?? "unknown";
								setNote({
									ok: false,
									text: tt("tex.failed").replace("{detail}", detail)
								});
			...
			(0, react.useEffect)(() => {
				...
				postWorkbench("/overleaf/workbench/bib-files", { sessionId }, 2e4).then((result) => {
					if (disposed) return;
			...
			(0, react.useEffect)(() => {
				...
				texSyncPendingRef.current = void 0;
				setTexBusy(false);
				if (texSyncTimerRef.current !== void 0) window.clearTimeout(texSyncTimerRef.current);
				texSyncTimerRef.current = void 0;
				if (sessionId === void 0) {
				...
				postWorkbench("/overleaf/workbench/tex-files", { sessionId }, 2e4).then((result) => {
					if (disposed) return;
					const files = Array.isArray(result.files) ? result.files.filter((path) => typeof path === "string") : [];
			...
			(0, react.useEffect)(() => () => {
				if (bibSyncTimerRef.current !== void 0) window.clearTimeout(bibSyncTimerRef.current);
				if (texSyncTimerRef.current !== void 0) window.clearTimeout(texSyncTimerRef.current);
				texSyncPendingRef.current = void 0;
			}, []);
			const syncLocalBib = (0, react.useCallback)(() => {
				const requestedPath = bibPath.trim();
				if (requestedPath === "") {
					setNote({
						ok: false,
						text: tt("bib.pathRequired")
					});
					return;
				}
				if (sessionId === void 0) {
					setNote({
						ok: false,
						text: tt("bib.noWorkspace")
					});
					return;
				}
				if (bibBusy) return;
				setBibBusy(true);
				postWorkbench("/overleaf/workbench/read-bib-file", {
					path: requestedPath,
					sessionId
				}, 2e4).then((file) => {
					...
					bibSyncTimerRef.current = window.setTimeout(() => {
						...
						setNote({
							ok: false,
							text: tt("bib.failed").replace("{detail}", "timeout")
						});
					}, 9e4);
				}).catch((error) => {
					setBibBusy(false);
					setNote({
						ok: false,
						text: tt("bib.failed").replace("{detail}", String(error))
					});
				});
			}, [
				bibBusy,
			...
			const startTexSync = (0, react.useCallback)(() => {
				if (sessionId === void 0 || sessionId === "") {
					setNote({
						ok: false,
						text: tt("tex.noWorkspace")
					});
					return;
				}
				if (texBusy) return;
				...
				if (texDirection === "local-to-overleaf" && texPath.trim() === "" && texManualContent === "") {
					setNote({
						ok: false,
						text: tt("tex.pathOrContentRequired")
					});
					return;
				}
				...
				texSyncPendingRef.current = {
					requestId,
					sessionId,
					direction: texDirection,
					path: texPath.trim(),
					manualText: texManualContent
				};
				setTexBusy(true);
				sendToFrame({
					type: "tex-document-request",
					requestId
				});
				texSyncTimerRef.current = window.setTimeout(() => {
			...
			const renderBibUpdater = (listId) => {
				const detectionText = bibScanState === "scanning" ? tt("bib.scanning") : bibCandidates.length > 0 ? tt("bib.detected").replace("{count}", String(bibCandidates.length)) : bibScanState === "unavailable" ? tt("bib.noWorkspace") : tt("bib.notDetected");
				return (0, react_jsx_runtime.jsxs)("div", {
					className: "dso-tool-section",
					children: [
						(0, react_jsx_runtime.jsx)("div", {
							className: "dso-muted",
							style: { fontWeight: 600 },
							children: tt("bib.title")
						}),
						...
						(0, react_jsx_runtime.jsx)("datalist", {
							id: listId,
							children: bibCandidates.map((path) => (0, react_jsx_runtime.jsx)("option", { value: path }, path))
						}),
						(0, react_jsx_runtime.jsx)("div", {
							className: "dso-muted",
							children: detectionText
						}),
						(0, react_jsx_runtime.jsx)("button", {
							className: "dso-btn",
							disabled: bibBusy || bibPath.trim() === "" || !cursorInsertEnabled || sessionId === void 0,
							onClick: syncLocalBib,
							children: bibBusy ? tt("bib.updating") : tt("bib.update")
						}),
						(0, react_jsx_runtime.jsx)("div", {
							className: "dso-muted",
							children: tt("bib.hint")
						})
					]
				});
			};
			const renderTexSync = () => {
				const detectionText = texScanState === "scanning" ? tt("tex.scanning") : texCandidates.length > 0 ? tt("tex.detected").replace("{count}", String(texCandidates.length)) : texScanState === "unavailable" ? tt("tex.noWorkspace") : tt("tex.notDetected");
				...
				return (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						(0, react_jsx_runtime.jsx)("div", {
							className: "dso-muted",
							style: { fontWeight: 600 },
							children: tt("tex.title")
						}),
						(0, react_jsx_runtime.jsx)("div", {
							className: "dso-muted",
							children: tt("tex.description")
						}),
						(0, react_jsx_runtime.jsxs)("div", {
							...
							role: "group",
							"aria-label": tt("tex.direction"),
							children: [(0, react_jsx_runtime.jsx)("button", {
								className: "dso-btn",
								"data-active": !reverse ? "1" : void 0,
								disabled: texBusy,
								onClick: () => {
									setTexDirection("overleaf-to-local");
									setTexReverseConfirmed(false);
								},
								children: tt("tex.directionPull")
							}), (0, react_jsx_runtime.jsx)("button", {
								className: "dso-btn",
								"data-active": reverse ? "1" : void 0,
								disabled: texBusy,
								onClick: () => {
									setTexDirection("local-to-overleaf");
									setTexReverseConfirmed(false);
								},
								children: tt("tex.directionPush")
							})]
						}),
						(0, react_jsx_runtime.jsx)("label", {
							className: "dso-muted",
							htmlFor: "dso-tex-sync-path",
							children: tt("tex.pathLabel")
						}),
						(0, react_jsx_runtime.jsx)("input", {
						...
						(0, react_jsx_runtime.jsx)("datalist", {
							id: "dso-tex-sync-candidates",
							children: texCandidates.map((path) => (0, react_jsx_runtime.jsx)("option", { value: path }, path))
						}),
						(0, react_jsx_runtime.jsx)("div", {
							className: "dso-muted",
							children: detectionText
						}),
						!reverse && texCandidates.length === 0 && texScanState === "done" && (0, react_jsx_runtime.jsx)("div", {
							className: "dso-muted",
							children: tt("tex.createHint")
						}),
						reverse && (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
							(0, react_jsx_runtime.jsx)("div", {
								className: "dso-muted",
								role: "alert",
								style: {
								...
								children: tt("tex.reverseWarning")
							}),
							(0, react_jsx_runtime.jsx)("label", {
								className: "dso-muted",
								htmlFor: "dso-tex-manual-content",
								children: tt("tex.manualLabel")
							}),
							...
							(0, react_jsx_runtime.jsxs)("label", {
								...
								}), (0, react_jsx_runtime.jsx)("span", { children: tt("tex.reverseConfirm") })]
							})
						] }),
						(0, react_jsx_runtime.jsx)("button", {
							className: `dso-btn ${reverse ? "dso-btn-primary" : ""}`,
							disabled: texBusy || sessionId === void 0 || reverse && !texReverseConfirmed,
							onClick: startTexSync,
							children: texBusy ? tt("tex.syncing") : reverse ? tt("tex.pushAction") : tt("tex.pullAction")
						}),
						(0, react_jsx_runtime.jsx)("div", {
							className: "dso-muted",
							children: reverse ? tt("tex.pushHint") : tt("tex.pullHint")
						})
					]
				});
			};
										...
										children: [
											...
											panelTab === "selection" && (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
												...
												(0, react_jsx_runtime.jsxs)("label", {
													...
													}), (0, react_jsx_runtime.jsx)("span", { children: tt("selection.safetyToggle") })]
												}),
												(0, react_jsx_runtime.jsx)("button", {
													className: "dso-btn dso-btn-primary",
												...
												(0, react_jsx_runtime.jsx)("div", {
													className: "dso-muted",
													style: !selectionSafetyEnabled ? { color: "var(--dsw-alias-state-error-primary, #c0392b)" } : void 0,
													children: selectionSafetyEnabled ? tt("selection.safety") : tt("selection.forceWarning")
												}),
											...
											panelTab === "compile" && (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
												...
												compileInfo === void 0 ? (0, react_jsx_runtime.jsx)("div", {
													...
													compileInfo.files.some((file) => file.error !== void 0) && (0, react_jsx_runtime.jsx)("div", {
														className: "dso-muted",
														style: { color: "var(--dsw-alias-state-error-primary, #c0392b)" },
														children: tt("compile.logReadFailed").replace("{detail}", compileInfo.files.filter((file) => file.error !== void 0).map((file) => `${file.path}: ${file.error ?? ""}`).join("; ").slice(0, 240))
													}),
													(0, react_jsx_runtime.jsx)("div", {
														className: "dso-muted",
														style: { fontWeight: 600 },
													...
													compileInfo.items.length === 0 ? compileInfo.files.length === 0 ? (0, react_jsx_runtime.jsx)("div", {
														className: "dso-muted",
														children: tt("compile.empty")
													}) : compileInfo.files.some((file) => file.error !== void 0) ? null : (0, react_jsx_runtime.jsx)("div", {
														className: "dso-muted",
											...
											panelTab === "outline" && (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
												...
												outlineItems === void 0 ? (0, react_jsx_runtime.jsx)("div", {
													...
													onClick: () => {
														if (item.line !== void 0 || item.text !== void 0) sendToFrame({
															type: "reveal",
															query: item.text,
															line: item.line
														});
...
`;
		...
		*/
		const CONFIG_NS_CANDIDATES = ["overleaf-workbench", "dsh-overleaf"];
		/**
		...
		*/
		function makeConfigScope(forms) {
			const formAt = (ns) => {
				try {
					return forms.get(ns) ?? void 0;
				} catch {
					return;
				}
			};
			const readyForm = () => {
				for (const ns of CONFIG_NS_CANDIDATES) try {
					const form = formAt(ns);
					if (form?.getSnapshot?.().status === "ready") return form;
				} catch {}
			};
			const anyForm = () => readyForm() ?? formAt(CONFIG_NS_CANDIDATES[0]) ?? formAt(CONFIG_NS_CANDIDATES[1]);
			return {
				getSnapshot: () => {
					try {
				...
				unset: async (field) => {
					const form = anyForm();
					if (form?.unset === void 0) return false;
		...
		*/
		function apply(ctx) {
			try {
				...
				if (configForms !== void 0 && typeof configForms.get === "function") try {
					const scope = makeConfigScope(configForms);
					const card = (props) => props.view === "summary" ? null : (0, react.createElement)(OverleafSettingsCard, props);
					const label = () => String(rawT("set.title"));
